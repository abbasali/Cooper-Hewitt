...
if (is_null(theme_get_setting('preset_style'))) {  
  ...
  global $theme_key;
	...
	if (!(function_exists('cooperhewitt_settings_defaults'))){
		include('theme-settings.php');
	}
	
	
  $defaults = cooperhewitt_settings_defaults();
...
*/
function cooperhewitt_preprocess_search_block_form(&$variables) {
...
function cooperhewitt_blocks($region) {
  $output = '';
...
function cooperhewitt_preprocess_block(&$variables){
...
function cooperhewitt_preprocess_maintenance_page(&$vars) {
	cooperhewitt_preprocess_page($vars);
}


function cooperhewitt_preprocess_page(&$variables) {
	...
	if( isset( $_COOKIE['cooperhewitt_preset_style'] ) )
		$variables['cooperhewitt_preset_style'] = $_COOKIE['cooperhewitt_preset_style']; 
	else
		$variables['cooperhewitt_preset_style'] = theme_get_setting(preset_style);

	
	if( isset( $_COOKIE['cooperhewitt_bg_style'] ) )
		$variables['cooperhewitt_bg_style'] = $_COOKIE['cooperhewitt_bg_style']; 
	else
		$variables['cooperhewitt_bg_style'] =  theme_get_setting(bg_style);
	
	if( isset( $_COOKIE['cooperhewitt_link_color'] ) )
		$variables['cooperhewitt_link_color'] = $_COOKIE['cooperhewitt_link_color']; 
	else
		$variables['cooperhewitt_link_color'] =  theme_get_setting(link_color);

	if( isset( $_COOKIE['cooperhewitt_font_family'] ) )
		$variables['cooperhewitt_font_family'] = $_COOKIE['cooperhewitt_font_family']; 
	else
		$variables['cooperhewitt_font_family'] = theme_get_setting(font_family);
	
	if( isset( $_COOKIE['cooperhewitt_defaultfont'] ) )
		$variables['cooperhewitt_defaultfont'] = $_COOKIE['cooperhewitt_defaultfont']; 
	else
		$variables['cooperhewitt_defaultfont'] = theme_get_setting(default_font);
	

		
	
	// set global for menu style if exists
	if( isset( $_COOKIE['cooperhewitt_menu_type'] ) )
		$variables['cooperhewitt_menu_type'] = $_COOKIE['cooperhewitt_menu_type']; 
	else
		$variables['cooperhewitt_menu_type'] = theme_get_setting('menu_type');
...
function cooperhewitt_change_theme($change, $changeVal, $bgVal='', $page=''){
	
	$theme_settings = variable_get('theme_cooperhewitt_settings', array());
	
	$cookie_prefix = "cooperhewitt_";
...
function main_menu_tree_output($tree) {
  ...
  if( isset( $_COOKIE['cooperhewitt_menu_type'] ) )
	$this_mtype = $_COOKIE['cooperhewitt_menu_type']; 
  else
...
function cooperhewitt_more_link($url, $title) {
...
function cooperhewitt_user_login_block($form) {
...
function cooperhewitt_button($element) {
...
function cooperhewitt_get_theme_headers($theme){
...
function cooperhewitt_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
 ...
 */
function cooperhewitt_comment_wrapper($content, $node) {
 ...
 * @ingroup themeable
 */
function cooperhewitt_menu_local_tasks() {
  return menu_primary_local_tasks();
}

function cooperhewitt_comment_submitted($comment) {
...
function cooperhewitt_node_submitted($node) {
  return t('!datetime — !username',
    array(
