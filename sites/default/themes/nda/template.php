...
if (is_null(theme_get_setting('preset_style'))) {  
  ...
  global $theme_key;
	...
	if (!(function_exists('nda_settings_defaults'))){
...
*/
function nda_preprocess_search_block_form(&$variables) {
...
function nda_preprocess_block(&$variables){
...
function nda_preprocess_maintenance_page(&$vars) {
	nda_preprocess_page($vars);
}


function nda_preprocess_page(&$variables) {
	...
	if( isset( $_COOKIE['nda_preset_style'] ) )
		$variables['nda_preset_style'] = $_COOKIE['nda_preset_style']; 
	else
		$variables['nda_preset_style'] = theme_get_setting(preset_style);

	
	if( isset( $_COOKIE['nda_bg_style'] ) )
		$variables['nda_bg_style'] = $_COOKIE['nda_bg_style']; 
	else
		$variables['nda_bg_style'] =  theme_get_setting(bg_style);
	
	if( isset( $_COOKIE['nda_link_color'] ) )
		$variables['nda_link_color'] = $_COOKIE['nda_link_color']; 
	else
		$variables['nda_link_color'] =  theme_get_setting(link_color);

	if( isset( $_COOKIE['nda_font_family'] ) )
		$variables['nda_font_family'] = $_COOKIE['nda_font_family']; 
	else
		$variables['nda_font_family'] = theme_get_setting(font_family);
	
	if( isset( $_COOKIE['nda_defaultfont'] ) )
		$variables['nda_defaultfont'] = $_COOKIE['nda_defaultfont']; 
	else
		$variables['nda_defaultfont'] = theme_get_setting(default_font);
	

		
	
	// set global for menu style if exists
	if( isset( $_COOKIE['nda_menu_type'] ) )
		$variables['nda_menu_type'] = $_COOKIE['nda_menu_type']; 
	else
		$variables['nda_menu_type'] = theme_get_setting('menu_type');
...
function nda_change_theme($change, $changeVal, $bgVal='', $page=''){
	
	$theme_settings = variable_get('theme_nda_settings', array());
...
function main_menu_tree_output($tree) {
  ...
  if( isset( $_COOKIE['nda_menu_type'] ) )
	$this_mtype = $_COOKIE['nda_menu_type']; 
  else
...
function nda_more_link($url, $title) {
...
function nda_user_login_block($form) {
...
function nda_get_theme_headers($theme){
...
function nda_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
 ...
 */
function nda_comment_wrapper($content, $node) {
 ...
 * @ingroup themeable
 */
function nda_menu_local_tasks() {
  return menu_primary_local_tasks();
}

function nda_comment_submitted($comment) {
...
function nda_node_submitted($node) {
  return t('!datetime — !username',
    array(
